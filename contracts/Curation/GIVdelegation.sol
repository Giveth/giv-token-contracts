...
contract GIVdelegation is GIVpower {
    ...
    function initialize(uint256 _initialDate, uint256 _roundDuration, address _tokenManager, address _tokenDistribution, uint256 _duration) initializer public {
    ...
    function lock(uint256 _amount, uint256 _rounds) public override {
        address delegate = delegateOf(msg.sender);
    ...
    function delegateOf(address _account) public returns (address) {
        return _delegateOf[_account];
    }

    function delegatedBalanceOf(address _account) public returns (uint256) {
