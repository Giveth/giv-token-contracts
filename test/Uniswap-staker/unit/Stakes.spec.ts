...
describe("unit/Stakes", () => {
    ...
    describe("#stakeToken", () => {
        ...
        describe("works and", () => {
            ...
            it("sets the stake struct properly", async () => {
                ...
                const depositStakesBefore = (
                    await context.staker.deposits(tokenId)
                ).numberOfStakes;
                await subject(tokenId, lpUser0);
                const stakeAfter = await context.staker.stakes(
                    tokenId,
                    incentiveId,
                );
                const depositStakesAfter = (
                    await context.staker.deposits(tokenId)
                ).numberOfStakes;

                expect(stakeBefore.secondsPerLiquidityInsideInitialX128).to.eq(
                    0,
                );
                expect(stakeBefore.liquidity).to.eq(0);
                expect(depositStakesBefore).to.eq(0);
                expect(
                    stakeAfter.secondsPerLiquidityInsideInitialX128,
                ).to.be.gt(0);
        ...
        describe("fails when", () => {
            ...
            it("you are not the owner of the deposit", async () => {
                ...
                await expect(
                    subject(tokenId, actors.lpUser2()),
                ).to.be.revertedWith(
            ...
            it("has 0 liquidity in the position", async () => {
                ...
                const tokenId2 = await mintPosition(
                    context.nft.connect(lpUser0),
                    {
                        token0: context.token0.address,
                        token1: context.token1.address,
                        fee: FeeAmount.MEDIUM,
                ...
                await context.nft.connect(lpUser0).decreaseLiquidity({
                    tokenId: tokenId2,
                    liquidity: (
                        await context.nft.positions(tokenId2)
                    ).liquidity,
                    amount0Min: 0,
                    amount1Min: 0,
    ...
    describe("#getRewardInfo", () => {
        ...
        it("returns correct rewardAmount and secondsInsideX128 for the position", async () => {
            ...
            const rewardInfo = await context.staker
                .connect(lpUser0)
                .getRewardInfo(stakeIncentiveKey, tokenId);

            const { tickLower, tickUpper } = await context.nft.positions(
                tokenId,
            );
            ...
            expect(rewardInfo.secondsInsideX128).to.equal(
                expectedSecondsInPeriod,
            );
        });
        ...
        it("reverts if stake does not exist", async () => {
            ...
            await expect(
                context.staker
                    .connect(lpUser0)
                    .getRewardInfo(stakeIncentiveKey, "100"),
            ).to.be.revertedWith(
    ...
    describe("#claimReward", () => {
        ...
        beforeEach("setup", async () => {
            ...
            subject = (_token: string, _to: string, _amount: BigNumber) =>
                context.staker
                    .connect(lpUser0)
                    .claimReward(_token, _to, _amount);
        });
        ...
        describe("when requesting the full amount", () => {
            it("emits RewardClaimed event", async () => {
                ...
                await expect(
                    subject(rewardToken.address, lpUser0.address, BN("0")),
                )
        ...
        describe("when requesting a nonzero amount", () => {
            it("emits RewardClaimed event", async () => {
                const { rewardToken } = context;
                await expect(
                    subject(rewardToken.address, lpUser0.address, claimable),
                )
                    .to.emit(context.staker, "RewardClaimed")
                    .withArgs(lpUser0.address, claimable);
            });
    ...
    describe("#unstakeToken", () => {
        ...
        describe("works and", () => {
            it("decrements deposit numberOfStakes by 1", async () => {
                const { numberOfStakes: stakesPre } =
                    await context.staker.deposits(tokenId);
                await subject(lpUser0);
                const { numberOfStakes: stakesPost } =
                    await context.staker.deposits(tokenId);
            ...
            it("updates the stake struct", async () => {
                ...
                expect(stakeBefore.secondsPerLiquidityInsideInitialX128).to.gt(
                    0,
                );
                expect(stakeBefore.liquidity).to.gt(0);
                expect(stakeAfter.secondsPerLiquidityInsideInitialX128).to.eq(
                    0,
                );
    ...
    describe("liquidityIfOverflow", () => {
        ...
        it("works when no overflow", async () => {
            ...
            await context.staker
                .connect(lpUser0)
                .stakeToken(incentiveResultToStakeAdapter(incentive), tokenId);
            const { liquidity } = await context.staker.stakes(
                tokenId,
                incentiveId,
            );
