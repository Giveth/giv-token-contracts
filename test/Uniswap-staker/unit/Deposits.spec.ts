...
describe("unit/Deposits", () => {
    ...
    describe("nft#safeTransferFrom", () => {
        ...
        it("allows depositing and staking for a single incentive", async () => {
            ...
            expect(stake.secondsPerLiquidityInsideInitialX128).not.to.eq(
                BN("0"),
            );
        });
        ...
        it("allows depositing and staking for two incentives", async () => {
            ...
            expect(stake2.secondsPerLiquidityInsideInitialX128).not.to.eq(
                BN("0"),
            );
        });
    ...
    describe("#onERC721Received", () => {
        ...
        beforeEach("set up position", async () => {
            ...
            const incentiveKey: ContractParams.IncentiveKey =
        ...
        describe("on successful transfer with staking data", () => {
            ...
            it("properly stakes the deposit in the select incentive", async () => {
                ...
                expect(
                    stakeBefore.secondsPerLiquidityInsideInitialX128,
                ).to.equal(0);
                expect(
    ...
    describe("#transferDeposit", () => {
        ...
        it("transfers nft ownership", async () => {
            const { owner: ownerBefore } = await context.staker.deposits(
                tokenId,
            );
            await context.staker
                .connect(lpUser0)
                .transferDeposit(tokenId, lpUser1.address);
            const { owner: ownerAfter } = await context.staker.deposits(
                tokenId,
            );
