...
describe("integration", async () => {
    ...
    describe("there are three LPs in the same range", async () => {
        ...
        describe("who all stake the entire time ", () => {
            it("allows them all to withdraw at the end", async () => {
                ...
                const { amountReturnedToCreator } =
                    await helpers.endIncentiveFlow({
                        createIncentiveResult,
                    });
                expect(rewardsEarned.add(amountReturnedToCreator)).to.eq(
                    totalReward,
                );
            });
            ...
            describe("time goes past the incentive end time", () => {
                it("still allows an LP to unstake if they have not already", async () => {
                    ...
                    const actions = {
                        doUnstake: (
                            params: HelperTypes.MintDepositStake.Result,
                        ) =>
                            staker
                                .connect(params.lp)
                                .unstakeToken(
        ...
        describe("when one LP unstakes halfway through", () => {
            it("only gives them one sixth the total reward", async () => {
                ...
                const unstakes: Array<HelperTypes.UnstakeCollectBurn.Result> =
                    [];
          ...
          (1/2 * 1/2) from after lpUser0. */
                ...
                expect(
                    ratioE18(unstakes[1].balance, unstakes[0].balance),
                ).to.eq("2.50");
                expect(
                    ratioE18(unstakes[2].balance, unstakes[1].balance),
                ).to.eq("1.00");

                // All should add up to totalReward
                expect(
                    bnSum(unstakes.map((u) => u.balance)).add(
                        amountReturnedToCreator,
                    ),
                ).to.eq(totalReward);
            });
        ...
        describe("when another LP starts staking halfway through", () => {
            describe("and provides half the liquidity", () => {
                it("gives them a smaller share of the reward", async () => {
                    const { helpers, createIncentiveResult, stakes, context } =
                        subject;
                    const { startTime, endTime } = createIncentiveResult;
                    ...
                    expect(
                        ratioE18(unstakes[2].balance, unstakes[3].balance),
                    ).to.eq("4.34");
        ...
        describe("when another LP adds liquidity but does not stake", () => {
            it("still changes the reward amounts", async () => {
                const { helpers, createIncentiveResult, context, stakes } =
                    subject;
                 ...
                 * */
                const secondHalfRewards = totalReward
                    .div(BN("2"))
                    .mul("3")
                    .div("4");
                ...
                expect(amountReturnedToCreator.add(rewardsEarned)).to.eq(
                    totalReward,
                );
            });
        });
    ...
    describe("when there are different ranges staked", () => {
        ...
        it("rewards based on how long they are in range", async () => {
            ...
            const midpoint = await getCurrentTick(
