...
import { GIV, TokenDistroV1 } from "../../typechain-types";
...
describe("TokenDistroV1", () => {
    beforeEach(async () => {
        ...
        tokenDistroFactory = await ethers.getContractFactory(
            "ModifiedTokenDistroMock",
        );
    });
    ...
    it("should be able to transfer the balance - sendPraiseRewards", async () => {
        ...
        await expect(
            ...
            .to.emit(tokenDistro, "Allocate")
            .withArgs(multisigAddress, recipientAddress4, amountRecipient4)
            .to.emit(tokenDistro, "PraiseRewardPaid")
            .withArgs(multisigAddress);
        expect(
            (await tokenDistro.balances(recipientAddress1)).allocatedTokens,
        ).to.be.equal(amountRecipient1);
    ...
    it("shouldn't be able to change the address if it's the distributor", async () => {
        ...
        // );
        await expect(
            tokenDistro.connect(recipient2).changeAddress(recipientAddress3),
        ).to.be.revertedWith(
            "TokenDistro::transferAllocation: NO_ALLOCATION_TO_TRANSFER",
        );
        await expect(
            tokenDistro.connect(recipient3).changeAddress(multisig2Address),
        ).to.be.revertedWith(
    ...
    it("should be able to cancel an allocation", async () => {
        ...
        await expect(
            tokenDistro
                .connect(recipient1)
                .transferAllocation(recipientAddress1, recipientAddress2),
        ).to.be.revertedWith(
            "TokenDistro::transferAllocation: ONLY_ADMIN_ROLE",
        );
        // await expect(
        //     tokenDistro.transferAllocation(
        //         recipientAddress1,
        //         recipientAddress3,
        //     ),
        // ).to.be.revertedWith(
        //     "TokenDistro::transferAllocation: ADDRESS_ALREADY_IN_USE",
        // );
        await expect(
            tokenDistro.transferAllocation(recipientAddress1, multisig2Address),
        ).to.be.revertedWith(
            "TokenDistro::transferAllocation: DISTRIBUTOR_ROLE_NOT_A_VALID_ADDRESS",
        );
        ...
        await expect(
            ...
            .to.emit(tokenDistro, "ChangeAddress")
            .withArgs(recipientAddress3, recipientAddress4);
    });
    ...
    it("should revert if it's not cancellabe", async () => {
        ...
        ).to.be.revertedWith("TokenDistro::transferAllocation: NOT_CANCELABLE");
    });
});
